...
- **Read the card number masked or unmasked.** `************6037` is the usual form, but a statement need not print one: the final cycle of a replaced card carries no summary block at all, only the unmasked `4111-1111-1111-6037` heading the transaction pages with the cardholder's name run into it. Reading only the masked form filed that statement under a nameless third card. The groups must be written apart — sixteen consecutive digits also sit inside every `Ref No. : 74100000000000000000123`.
...
label says both things at once — `UOB ····4419 (was ····6037)` — so a folded row
