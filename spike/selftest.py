...
def test_row_parsing() -> None:
    ...
    masked = rows.parse_page("     UOB ONE CARD ************8251 EUGENE PHUA" + "\n",
                             ("2025-01-16", "2025-02-16"), 2025)
    check("a masked card number gives the last four",
          masked["account_last4"] == "8251", repr(masked["account_last4"]))
    bare = rows.parse_page("     4265-8840-8142-8251EUGENE PHUA (continued)" + "\n",
                           ("2025-01-16", "2025-02-16"), 2025)
    check("...and so does an unmasked one with a name run into it",
          bare["account_last4"] == "8251", repr(bare["account_last4"]))
    ...
    ref = rows.parse_page("                Ref No. : 74541835006288103325234" + "\n",
                          ("2025-01-16", "2025-02-16"), 2025)
...
def test_card_lineage() -> None:
    ...
    conn.row_factory = sqlite3.Row
    conn.executescript(db.SCHEMA)
    conn.execute("INSERT INTO account (id, issuer, kind, last4) VALUES (1, 'UOB', 'credit', '8251')")
    conn.execute("INSERT INTO account (id, issuer, kind, last4) VALUES (2, 'UOB', 'credit', '1208')")
    conn.execute("INSERT INTO account (id, issuer, kind, last4) VALUES (3, 'DBS', 'credit', '4685')")
    ...
    check("an unlinked card is its own lineage",
          sorted(db.coverage_by_account(conn)) == ["UOB ····1208", "UOB ····8251"],
    ...
    check("the label names the card it replaced",
          lineage[1]["label"] == "UOB ····1208 (was ····8251)", lineage[1]["label"])
    ...
    check("one card, one row on the timeline",
          sorted(cov) == ["UOB ····1208 (was ····8251)"], repr(sorted(cov)))
    check("and one unbroken window across the replacement",
          cov["UOB ····1208 (was ····8251)"] == [("2026-01-01", "2026-04-30")],
          repr(cov["UOB ····1208 (was ····8251)"]))
    ...
    check("one entry per lineage to filter by",
          db.card_options(conn) == {"UOB ····1208 (was ····8251)": 2, "DBS ····4685": 3},
    ...
    check("unlinking puts the card back on its own",
          ...
          and db.card_lineages(conn)[1]["label"] == "UOB ····8251",
